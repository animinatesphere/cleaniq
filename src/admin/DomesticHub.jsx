...
const DomesticHub = () => {
  ...
  const isDomestic = (b) => {
    ...
    return (
      ...
      (!s.includes("commercial") && !s.includes("office") && !s.includes("airbnb") && !s.includes("shortlet"))
    );
  };

  const q = search.trim().toLowerCase();
  const results = q.length < 2 ? [] : allBookings
    ...
    .filter((b) => {
      ...
      return name.includes(q) || email.includes(q) || ref.includes(q) || addr.includes(q);
    })
    .slice(0, 8);

  const pick = (b) => {
    setSelected(b);
    setSearch(`${b.customer?.firstName || ""} ${b.customer?.lastName || ""} — ${b.bookingId}`);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-primary-dark tracking-tight">Domestic Hub</h1>
          <p className="text-sm text-slate-500 font-medium mt-0.5">Search a domestic booking to generate a property condition report</p>
        </div>
      </div>

      {/* Booking search */}
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Select Booking</p>
        <div className="relative" ref={searchRef}>
          ...
          {showResults && q.length >= 2 && results.length === 0 && (
            <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-2xl shadow-xl z-30 px-5 py-4">
              <p className="text-sm text-slate-400 font-medium">No domestic bookings match "{search}"</p>
            </div>
          )}
        </div>
        ...
        {selected && (
          <div className="mt-4 flex flex-wrap gap-4 px-4 py-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs">
            <span className="font-black text-emerald-800">{selected.customer?.firstName} {selected.customer?.lastName}</span>
            <span className="text-emerald-700 font-medium">{selected.bookingId}</span>
            <span className="text-emerald-700 font-medium">{selected.service}</span>
            <span className="text-emerald-700 font-medium">{fmt(selected.schedule?.date)}</span>
            {selected.details?.address && <span className="text-emerald-600 font-medium">{selected.details.address}</span>}
            <button
      ...
      {selected ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-slate-300">
          <Home size={48} />
          <p className="text-sm font-bold text-slate-400">Search and select a booking above to begin</p>
        </div>
      )}
    </div>
  );
