...
import {
  ...
  Building2, AlignLeft, Users, RefreshCw,
} from "lucide-react";
...
// Mounts parsed HTML as a div in the main document for html2canvas
...
const InvoiceBuilder = () => {
  ...
  const handleLoadBooking = async () => {
    ...
    try {
      ...
      const mainDesc = [data.service || "Cleaning Service", data.details?.bedrooms ? `${data.details.bedrooms} bed` : "", data.payment?.billingType !== "flat" && data.details?.duration ? `${data.details.duration} hrs` : ""].filter(Boolean).join(" · ");
      setItems([
        { id: uid(), description: mainDesc, qty: 1, rate: Number(data.payment?.amount || 0) },
      ...
      setToast({ msg: err.message || "Could not find that booking", type: "error" });
  ...
  const handleGeneratePaymentLink = async () => {
    if (subtotal <= 0) { setToast({ msg: "Add items with an amount first", type: "error" }); return; }
    ...
    } catch (err) { setToast({ msg: err.message || "Failed to generate link", type: "error" }); }
    finally { setGeneratingLink(false); }
  };

  const handleSend = async () => {
    if (!client.email || !items[0]?.description) { setToast({ msg: "Add a customer email and at least one item", type: "error" }); return; }
    setSending(true);
    try {
      ...
      setToast({ msg: loadedBookingId ? `Invoice sent to ${client.email} — booking marked Confirmed` : `Invoice sent to ${client.email}`, type: "success" });
    } catch (err) { setToast({ msg: err.message || "Failed to send invoice", type: "error" }); }
...
<div style="background:#f0fdf4;border-radius:10px;padding:14px 18px;border-left:4px solid #0A5C43;margin-bottom:16px">
  ...
  <p style="font-size:12px;color:#065f46;white-space:pre-line;line-height:1.7;margin:0">${notes}</p>
    ...
    // Scoped CSS so styles don't leak into main page while temporarily mounted
    const CSS = `
#inv-root *{margin:0;padding:0;box-sizing:border-box}
#inv-root{font-family:Arial,Helvetica,sans-serif;font-size:12px;color:#1e293b;background:#fff;width:794px;-webkit-print-color-adjust:exact;print-color-adjust:exact}`;
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f0fdf4;border-top:3px solid #0A5C43">
  <tr>
    <td style="padding:12px 20px;border-right:1px solid #bbf7d0;width:25%">
      <p style="font-size:7.5px;font-weight:800;color:#0A5C43;text-transform:uppercase;letter-spacing:1.2px;opacity:.75;margin-bottom:3px">Invoice #</p>
      <p style="font-size:11px;font-weight:700;color:#0f172a">${invoiceNumber}</p>
    </td>
    <td style="padding:12px 20px;border-right:1px solid #bbf7d0;width:25%">
      <p style="font-size:7.5px;font-weight:800;color:#0A5C43;text-transform:uppercase;letter-spacing:1.2px;opacity:.75;margin-bottom:3px">Invoice Date</p>
      <p style="font-size:11px;font-weight:700;color:#0f172a">${invoiceDate}</p>
    </td>
    <td style="padding:12px 20px;border-right:1px solid #bbf7d0;width:25%">
      <p style="font-size:7.5px;font-weight:800;color:#0A5C43;text-transform:uppercase;letter-spacing:1.2px;opacity:.75;margin-bottom:3px">Service Date</p>
      <p style="font-size:11px;font-weight:700;color:#0f172a">${serviceDate || "—"}</p>
    </td>
    <td style="padding:12px 20px;width:25%">
      <p style="font-size:7.5px;font-weight:800;color:#0A5C43;text-transform:uppercase;letter-spacing:1.2px;opacity:.75;margin-bottom:3px">Status</p>
      <p style="font-size:11px;font-weight:800;color:${showPaidBadge ? "#16a34a" : "#d97706"}">${showPaidBadge ? "✓ Paid" : "Payment Due"}</p>
    </td>
  </tr>
</table>

<!-- BODY -->
<div style="padding:32px 44px 40px;background:#ffffff">
...
<div style="background:#0f172a;padding:18px 44px;text-align:center">
  ...
  const renderSectionForm = (id) => {
    switch (id) {
          ...
          <div className="space-y-2">
            ...
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-[1fr_50px_66px_34px] gap-1.5">
                ...
                <input type="number" min="1" value={item.qty} onChange={(e) => updateItem(item.id, "qty", e.target.value)} className={`${inp} text-center`} />
      ...
      case "bank":
        return (
          <div className="grid grid-cols-2 gap-2">
            <Field label="Account Name" span={2}><input className={inp} value={bank.accountName} onChange={(e) => setB("accountName", e.target.value)} placeholder="Account Name" /></Field>
            ...
            <Field label="Account Number"><input className={inp} value={bank.accountNumber} onChange={(e) => setB("accountNumber", e.target.value)} placeholder="00000000" /></Field>
          ...
          <div>
            {paymentLink ? (
              <div className="flex gap-2 items-center">
                ...
                <button onClick={() => { navigator.clipboard.writeText(paymentLink); setToast({ msg: "Link copied", type: "success" }); }} className="p-2.5 rounded-lg border border-indigo-200 text-indigo-600 hover:bg-indigo-100 transition-all"><Copy size={14} /></button>
        ...
        return (
          <div key={id}>
              ...
              <thead>
                <tr className="bg-slate-900">
                  {["Description", "Qty", "Rate", "Amount"].map((h, i) => (
                    <th key={h} className={`p-2 font-bold uppercase text-[7px] text-emerald-300 tracking-wide ${i === 0 ? "text-left" : "text-right"}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.map((item, i) => (
                  <tr key={item.id} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="p-2 text-right border-b border-slate-100 text-slate-500">{item.qty}</td>
                    <td className="p-2 text-right border-b border-slate-100 text-slate-500">£{Number(item.rate || 0).toFixed(2)}</td>
            ...
            <div className="flex justify-end mt-2">
              <div className="bg-[#0A5C43] rounded-xl px-4 py-2">
                <span className="font-black text-sm text-white">£{subtotal.toFixed(2)}</span>
                <span className="text-[9px] text-emerald-200 ml-1.5">{showPaidBadge ? "Paid" : "Due"}</span>
              </div>
            </div>
          </div>
        );
        ...
        return bankText ? (
          <div key={id} className="rounded-xl overflow-hidden border border-slate-200">
            <div className="bg-slate-900 px-3 py-1.5">
              <p className="text-[7px] font-black text-emerald-300 uppercase tracking-wide">Bank Transfer Details</p>
            </div>
            <div className="p-3 bg-white">
              <p className="whitespace-pre-line text-[9px] text-slate-700 font-medium leading-loose">{bankText}</p>
            </div>
          </div>
        ) : null;
  ...
  return (
    <div className="space-y-5 pb-24">
      ...
      {/* Load from booking + invoice number */}
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-sm p-5 space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleLoadBooking} disabled={loadingBooking} className="flex items-center gap-2 px-4 rounded-xl bg-slate-900 text-white font-semibold text-sm hover:bg-slate-800 transition-all disabled:opacity-60">
      ...
      <div className="grid lg:grid-cols-[1fr_1.2fr] gap-6">
        ...
        <div className="space-y-2">
              ...
              >
                <div className="flex items-center gap-2.5 px-3.5 py-3 cursor-pointer select-none" onClick={() => sec.enabled && toggleOpen(sec.id)}>
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all ${sec.enabled ? "text-primary hover:bg-primary/10" : "text-slate-300 hover:bg-slate-100"}`}
                  >
          ...
          <div className="flex gap-3 pt-3">
            ...
            <button
              onClick={handleSend}
              disabled={sending}
              className="flex-1 flex items-center justify-center gap-2 px-5 py-3 rounded-xl bg-primary text-white hover:bg-primary/90 font-semibold text-sm shadow-sm transition-all disabled:opacity-60"
            >
        ...
        <div className="bg-slate-100 border border-slate-200/80 rounded-2xl shadow-sm p-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden">
            ...
            <div style={{ background: "#0A5C43" }} className="px-5 py-4">
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  ...
                  {showPaidBadge && (
                    <span className="inline-block mt-2 px-2.5 py-0.5 rounded-full bg-emerald-300 text-emerald-950 text-[8px] font-black uppercase">✓ Paid</span>
                  )}
                </div>
              </div>
            </div>
            ...
            {/* Preview meta strip */}
            <div className="grid grid-cols-4 text-[8px] border-b-2 border-t-0" style={{ background: "#f0fdf4", borderTop: "3px solid #0A5C43" }}>
