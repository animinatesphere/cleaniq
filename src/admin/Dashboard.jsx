      ...
      try {
        ...
        setData({
          bookings: bookings.slice(0, 5).map((b) => ({
            ...
            service: b.service,
            date: new Date(b.schedule.date).toLocaleDateString("en-GB", {
              day: "numeric",
              month: "short",
            }),
            status: b.status,
            amount: `£${b.payment.amount.toLocaleString()}`,
            region: b.region,
            color:
              b.status === "Confirmed"
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {data.stats.map((stat, i) => (
          <div
            key={i}
            className="bg-white p-6 rounded-4xl border border-slate-200 shadow-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 group"
          >
            <div className="flex justify-between items-start mb-6">
              ...
              <div
                className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded-lg ${stat.isUp ? "bg-emerald-50 text-emerald-600" : "bg-rose-50 text-rose-600"}`}
              >
                {stat.isUp ? (
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200 rounded-[32px] lg:rounded-[40px] shadow-sm overflow-hidden flex flex-col">
          <div className="p-5 md:p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-white border border-slate-200 text-sm font-bold text-primary-dark hover:bg-slate-50 transition-all"
            >
              Manage All
            </button>
          </div>
              ...
              <tbody className="divide-y divide-slate-50">
                {data.bookings.map((item, i) => (
                  <tr
                    key={i}
                    onClick={() =>
                      navigate(`/admin/bookings?id=${item.mongoId}`)
                    }
                    className="group hover:bg-slate-50/80 transition-colors cursor-pointer"
                  >
                    <td className="px-8 py-5">
                      <p className="font-bold text-primary-dark group-hover:text-primary transition-colors">
                        {item.name}
                      </p>
                    ...
                    <td className="px-4 py-5">
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border shadow-sm ${item.color}`}
                      >
                        {item.status}
                      </span>
                    </td>
                    <td className="px-8 py-5 text-right font-black text-primary-dark">
                      {item.amount}
                    </td>
                  </tr>
                ))}
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white border border-slate-200 p-6 md:p-8 rounded-[32px] md:rounded-[40px] shadow-sm">
            <h3 className="text-lg font-black text-primary-dark mb-6">
              Marketing Tools
            </h3>
