...
function buildPreviewHtml(rawBody, subject) {
  ...
  const replySubject = encodeURIComponent(
    "Re: " + (subject || "Your message").replace(/\{\{[^}]+\}\}/g, "…"),
  );
...
function PreviewModal({ emailStep, emailStyle, onClose }) {
  ...
  const sample = {
    ...
    "{{last_name}}": "Wilson",
    "{{company}}": "ABC Ltd",
    "{{email}}": "james@abcltd.com",
    "{{domain}}": "abcltd.com",
  };
  const replace = (t) =>
    Object.entries(sample).reduce((s, [k, v]) => s.replaceAll(k, v), t || "");
  const sub = replace(emailStep.subject);
  const body = replace(emailStep.body);
  const html =
    emailStyle === "plain"
      ? buildPreviewHtmlPlain(body)
      : buildPreviewHtml(body, sub);

  return (
    <div
      className="fixed inset-0 z-[80] flex items-stretch justify-center bg-black/90 backdrop-blur-md p-0 sm:p-6 sm:items-center"
      onClick={onClose}
    >
      <div
        className="w-full sm:max-w-[760px] sm:max-h-[94vh] flex flex-col bg-[#050e09] sm:rounded-2xl overflow-hidden shadow-2xl border border-white/[0.08]"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07] shrink-0 bg-white/[0.02]">
          <div>
            ...
            <p className="text-white/30 text-[11px] mt-0.5">
              Sample data: James Wilson · ABC Ltd
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg bg-white/[0.06] flex items-center justify-center text-white/40 hover:text-white cursor-pointer transition-colors"
            >
              <X size={14} />
            </button>
          </div>
          ...
          <div className="flex flex-col flex-1 min-h-0">
            ...
            <div className="shrink-0 bg-[#f6f8fc] border-b border-gray-200/80 px-5 py-4">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-10 h-10 rounded-full bg-[#073d27] flex items-center justify-center text-white font-black text-sm shrink-0">
                  CS
                </div>
                <div>
...
function CampaignDetail({ campaignId, onBack, onRefresh, onEditCampaign }) {
  ...
  const loadSends = useCallback(async () => {
    ...
    const r = await afetch(
      `${API}/cold-email/campaigns/${campaignId}/sends?${p}`,
    );
    const d = await r.json();
    setSends(d.sends || []);
  ...
  async function doAction(verb) {
    setActionBusy(verb);
    await afetch(`${API}/cold-email/campaigns/${campaignId}/${verb}`, {
      method: "POST",
    });
    await loadCampaign();
    onRefresh();
  ...
  async function checkReplies() {
    setCheckBusy(true);
    await afetch(`${API}/cold-email/campaigns/${campaignId}/check-replies`, {
      method: "POST",
    });
    await loadCampaign();
    await loadSends();
  ...
  if (!campaign)
    return (
      <div className="flex flex-col items-center justify-center py-32">
        ...
        <button
          onClick={onBack}
          className="mt-4 text-emerald-400 text-sm font-bold cursor-pointer hover:text-emerald-300"
        >
          ← Go back
        </button>
      </div>
  ...
  const replyRate =
    s.sent > 0 ? ((s.replied / s.sent) * 100).toFixed(1) : "0.0";
  ...
  const STAT_CARDS = [
    {
      icon: Send,
      label: "Emails Sent",
      value: (s.sent || 0).toLocaleString(),
      sub: `${deliveryRate}% of ${total.toLocaleString()}`,
      color: "text-white",
      bg: "bg-white/[0.04]",
    ...
    },
    {
      icon: MessageSquare,
      label: "Replied",
      value: (s.replied || 0).toLocaleString(),
    ...
    },
    {
      icon: Users,
      label: "Unsubscribed",
      value: (s.unsubscribed || 0).toLocaleString(),
      sub: "removed themselves",
    ...
    },
    {
      ...
      value: ((s.bounced || 0) + (s.failed || 0)).toLocaleString(),
      sub: "delivery errors",
      color: "text-rose-400",
      bg: "bg-rose-500/[0.05]",
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <span
            className={`inline-flex items-center gap-1.5 text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-wider ${st.badge}`}
          >
          ...
          ({ icon: Icon, label, value, sub, color, bg, border }) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <p className="text-white/35 text-xs font-bold uppercase tracking-wider">
                  {label}
                </p>
                <div
              ...
              <p className={`font-black text-3xl leading-none ${color}`}>
                {value}
              </p>
        ...
        <div className="bg-white/[0.025] border border-white/[0.06] rounded-xl px-4 py-3.5 flex items-center gap-4">
          ...
          <div className="flex-1 h-2 bg-white/[0.05] rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(100, parseFloat(replyRate))}%` }}
            />
          </div>
        ...
        {[
          ...
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-2 text-xs font-bold px-3 py-2.5 rounded-lg cursor-pointer transition-all ${
              tab === id
                ? "bg-[#061A13] text-white border border-white/[0.08] shadow-sm"
            ...
            <div className="divide-y divide-white/[0.04]">
              {[
                ...
                },
                {
                  ...
                  value: (
                    <span
                      className={`text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${STATUS_STYLES[campaign.status]?.badge}`}
                    >
                      {campaign.status}
                    </span>
                  ),
                ...
                {
                  label:
                    campaign.status === "scheduled" ? "Sends at" : "Launched",
                  value:
                    campaign.status === "scheduled" &&
                    campaign.scheduledStartAt ? (
                      new Date(campaign.scheduledStartAt).toLocaleDateString(
                        "en-GB",
                        {
                          day: "numeric",
                          month: "short",
                      ...
                      <span className="text-white/25 italic">not launched</span>
                    ),
                },
                {
                  label: "Contacts",
                  value: `${total.toLocaleString()} recipients`,
                },
                {
                  label: "Steps",
                  value: `${(campaign.steps || []).length} email${campaign.steps?.length > 1 ? "s" : ""} in sequence`,
                },
              ].map(({ label, value }) => (
                <div
                  key={label}
          ...
          {campaign.steps?.[0] && (
            <div className="bg-white/[0.025] border border-white/[0.06] rounded-2xl overflow-hidden flex flex-col">
              <div className="px-5 py-3.5 border-b border-white/[0.06] flex items-center justify-between">
                ...
                <button
                  onClick={() => setPreviewStep(campaign.steps[0])}
              ...
              <div className="flex-1 p-5 space-y-3">
                <div className="flex items-start gap-3">
                  <span className="text-white/25 text-xs font-bold uppercase tracking-wider w-14 shrink-0 pt-0.5">
                    Subject
                  </span>
                  <span className="text-white/85 text-sm font-semibold leading-relaxed">
                    {campaign.steps[0].subject || "(no subject)"}
                  </span>
                </div>
                <div className="flex items-start gap-3">
                  <span className="text-white/25 text-xs font-bold uppercase tracking-wider w-14 shrink-0 pt-0.5">
                  ...
                  <p className="text-white/45 text-sm leading-relaxed whitespace-pre-wrap line-clamp-6">
                    {campaign.steps[0].body || "(empty)"}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex gap-1.5 flex-wrap">
              {["", "pending", "sent", "replied", "failed", "skipped"].map(
                (st) => {
                  ...
                  return (
                    ...
                    >
                      {st ? info?.label || st : "All"}{" "}
                      {!st && sendsTotal > 0 && (
                        <span className="ml-1 opacity-60">{sendsTotal}</span>
                      )}
                    </button>
                  );
                },
          ...
          <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden">
            ...
            <div className="grid grid-cols-[auto_1fr_1fr_auto_auto] gap-4 px-4 py-2.5 border-b border-white/[0.06] bg-white/[0.02]">
              <div className="w-8" />
              <p className="text-white/25 text-[10px] font-bold uppercase tracking-wider">
                Name / Email
              </p>
              <p className="text-white/25 text-[10px] font-bold uppercase tracking-wider hidden sm:block">
                Company
              </p>
              <p className="text-white/25 text-[10px] font-bold uppercase tracking-wider">
                Status
                ...
                return (
                  ...
                  >
                    <div className="grid grid-cols-[auto_1fr_1fr_auto_auto] gap-4 px-4 py-3 hover:bg-white/[0.02] transition-colors items-center">
                      <div
                        className={`w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-black ${send.status === "replied" ? "bg-emerald-500/15 border border-emerald-500/25 text-emerald-400" : "bg-white/[0.06] border border-white/[0.07] text-white/40"}`}
                      >
                        {init}
                      </div>
                      ...
                      <p className="text-white/35 text-xs truncate hidden sm:block">
                        {c.company || "—"}
                      </p>
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[10px] font-black px-2 py-1 rounded-full uppercase tracking-wide whitespace-nowrap ${sInfo.color}`}
                        >
                          {sInfo.label}
                        </span>
                        {hasReply && (
                          <button
                            onClick={() =>
                              setViewReply(
                                viewReply?._id === send._id ? null : send,
                              )
                            }
                        ...
                        <div className="flex items-center gap-3 px-4 py-2.5 border-b border-emerald-500/15 bg-emerald-500/[0.04]">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-emerald-300 text-xs font-bold truncate">
                              {send.replySubject ||
                                `Re: ${(campaign?.steps?.[0]?.subject || "your email").replace(/\{\{[^}]+\}\}/g, "…")}`}
                            </p>
                            <p className="text-emerald-400/50 text-[10px]">
                              From {send.replyFrom || send.contactEmail}
                              {send.repliedAt &&
                                ` · ${new Date(send.repliedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}`}
                            </p>
                          </div>
                        </div>
                            ...
                            <div className="flex items-start gap-2.5">
                              ...
                              <div>
                                ...
                                <p className="text-white/35 text-xs mt-1">
                                  ...
                                  button at the top to fetch the reply content
                                  from Gmail. It will appear here after.
                                </p>
                              </div>
                            </div>
                          )}
          ...
          {sendsTotal > 50 && (
            <div className="flex items-center justify-between text-xs text-white/30">
              <span>
                {sendsTotal.toLocaleString()} total · page {sendsPage} of{" "}
                {Math.ceil(sendsTotal / 50)}
              </span>
              <div className="flex gap-1.5">
                <button
                  onClick={() => setSendsPage((p) => Math.max(1, p - 1))}
                  disabled={sendsPage === 1}
                  className="px-3 py-1.5 rounded-lg bg-white/[0.04] border border-white/[0.07] disabled:opacity-30 cursor-pointer disabled:cursor-default hover:bg-white/[0.08] transition-colors"
                >
                  ← Prev
                </button>
                <button
      ...
      {tab === "sequence" && (
        <div className="space-y-3">
          {(campaign.steps || []).map((step, i) => (
            <div
              key={i}
              className={`border rounded-2xl overflow-hidden transition-all ${expandedStep === i ? "border-emerald-500/30 bg-emerald-500/[0.04]" : "border-white/[0.07] bg-white/[0.025]"}`}
            >
              {/* Step header */}
              <div
              ...
              >
                <div
                  className={`w-8 h-8 rounded-xl flex items-center justify-center text-sm font-black shrink-0 ${i === 0 ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-white/[0.07] text-white/50 border border-white/[0.1]"}`}
                >
                  {i + 1}
                </div>
                <div className="flex-1 min-w-0">
                  <p
                    className={`font-bold text-sm truncate ${i === 0 ? "text-white" : "text-white/75"}`}
                  >
                  ...
                  <p className="text-white/25 text-[11px] mt-0.5">
                    {i === 0
                      ...
                      : `Sent ${step.waitDays || 1} day${step.waitDays !== 1 ? "s" : ""} after previous`}
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                  ...
                  {step.waitDays > 0 && (
                    <span className="text-white/25 text-xs bg-white/[0.05] border border-white/[0.07] px-2 py-1 rounded-lg">
                      +{step.waitDays}d
                    </span>
                  )}
                </div>
              ...
              {expandedStep === i && (
                <div className="px-5 pb-5">
                  ...
                  <p className="text-white/25 text-[10px] font-bold uppercase tracking-wider mb-2">
                    Message body
                  </p>
...
function Wizard({ onClose, onCreated, editCampaign }) {
  ...
  useEffect(() => {
    afetch(`${API}/cold-email/mailboxes`)
      .then((r) => r.json())
      .then(setMailboxes)
      .catch(() => {});
    afetch(`${API}/cold-email/templates`)
      .then((r) => r.json())
  ...
  const toggleMailbox = (id) =>
    setData((d) => ({
      ...d,
      mailboxIds: d.mailboxIds.includes(id)
        ? d.mailboxIds.filter((m) => m !== id)
        : [...d.mailboxIds, id],
    }));
  ...
  const toggleContact = (id) =>
    setData((d) => ({
      ...d,
      contactIds: d.contactIds.map(String).includes(String(id))
        ? d.contactIds.filter((c) => String(c) !== String(id))
        : [...d.contactIds, id],
    }));
  ...
  const saveTemplate = async () => {
    ...
    setSavingTpl(true);
    try {
      const r = await afetch(`${API}/cold-email/templates`, {
        method: "POST",
        body: JSON.stringify({ name: tplName.trim(), steps: data.steps }),
      });
  ...
  function validate() {
    ...
    if (step === 3 && !data.contactIds.length)
  ...
  async function save(launch = false) {
    ...
    try {
      ...
      if (editCampaign) {
        const r = await afetch(
          `${API}/cold-email/campaigns/${editCampaign._id}`,
          { method: "PUT", body: JSON.stringify(data) },
        );
        camp = await r.json();
      } else {
        const r = await afetch(`${API}/cold-email/campaigns`, {
          method: "POST",
      ...
      if (launch) {
        const body =
          scheduleMode && scheduleDate
            ? { scheduledStartAt: new Date(scheduleDate).toISOString() }
            : {};
        const r = await afetch(
          `${API}/cold-email/campaigns/${camp._id}/launch`,
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
        <div className="bg-[#060f09] border border-white/[0.08] rounded-2xl w-full max-w-[740px] max-h-[90vh] flex flex-col shadow-2xl">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06] shrink-0">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-xl bg-white/[0.04] flex items-center justify-center text-white/30 hover:text-white cursor-pointer transition-colors"
            >
              <X size={14} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-3 px-6 pt-5 pb-2 shrink-0">
            {LABELS.map((label, i) => (
              <div key={i} className="flex items-center flex-1 gap-2">
                ...
                <span
                  className={`text-xs font-bold hidden sm:block truncate ${i + 1 === step ? "text-white" : "text-white/25"}`}
                >
                  {label}
                </span>
                {i < LABELS.length - 1 && (
                  <div
                    className={`flex-1 h-px ${i + 1 < step ? "bg-emerald-500/40" : "bg-white/[0.06]"}`}
                  />
                )}
              </div>
            ))}
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <div className="flex-1 bg-emerald-500/[0.05] border border-emerald-500/[0.15] rounded-xl px-4 py-3 text-white/45 text-xs leading-relaxed">
                    Write the email your contacts receive. Use{" "}
                    <code className="text-emerald-400 bg-emerald-500/15 px-1.5 py-0.5 rounded">
                      {"{{first_name}}"}
                    </code>
                    ,{" "}
                    <code className="text-emerald-400 bg-emerald-500/15 px-1.5 py-0.5 rounded">
                    ...
                    to personalise. Unsubscribe link added automatically.
                  </div>
                  <button
                  ...
                  <div className="bg-white/[0.025] border border-white/[0.08] rounded-2xl overflow-hidden">
                    ...
                    {templates.length === 0 ? (
                      <p className="text-white/25 text-sm text-center py-8">
                        No saved templates yet — save one below.
                      </p>
                    ) : (
                ...
                {data.steps.map((s, idx) => (
                  <div
                    key={idx}
                    className="border border-white/[0.08] rounded-2xl overflow-hidden bg-white/[0.01]"
                  >
                    <div
                      className={`flex items-center justify-between px-4 py-3 border-b border-white/[0.07] ${idx === 0 ? "bg-emerald-500/[0.05]" : "bg-white/[0.025]"}`}
                    >
                      <div className="flex items-center gap-2.5">
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-black ${idx === 0 ? "bg-emerald-500/25 text-emerald-400" : "bg-white/[0.08] text-white/45"}`}
                        >
                          {idx + 1}
                        </div>
                        <span
                          className={`text-sm font-bold ${idx === 0 ? "text-emerald-300/90" : "text-white/60"}`}
                        >
                          {idx === 0 ? "First email" : `Follow-up ${idx}`}
                        </span>
                        {idx === 0 && (
                          <span className="text-emerald-400/35 text-xs hidden sm:inline">
                            · sent immediately
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-white/[0.05]">
                      <span className="text-white/20 text-xs font-bold uppercase tracking-wider w-14 shrink-0">
                        Subject
                      </span>
                      <input
                        type="text"
                        value={s.subject}
                        onChange={(e) =>
                          setStepField(idx, "subject", e.target.value)
                        }
                        placeholder={
                          idx === 0
                            ? "e.g. Quick question, {{first_name}}"
                            : "e.g. Following up, {{first_name}}"
                        }
                    ...
                    <div className="relative">
                      <textarea
                        value={s.body}
                        onChange={(e) =>
                          setStepField(idx, "body", e.target.value)
                        }
                        rows={9}
                        placeholder={
                      ...
                      <div className="absolute bottom-2.5 right-4 text-white/15 text-[11px] pointer-events-none select-none">
                        ...
                        words · {s.body.length} chars
                      </div>
                    </div>

                    {/* Variable chips */}
                    <div className="flex items-center gap-2 px-4 py-3 border-t border-white/[0.05] flex-wrap">
                      <span className="text-white/20 text-xs font-semibold shrink-0">
                        Insert:
                      </span>
                      {VARS.map((v) => (
                        <button
                      ...
                      onClick={() =>
                        setData((d) => ({
                          ...
                          steps: [
                            ...
                            {
                              order: 1,
                              subject: "Following up — " + d.steps[0].subject,
                            ...
                            {
                              order: 2,
                              subject: "Quick check-in — " + d.steps[0].subject,
                            ...
                            {
                              order: 3,
                              subject: "One last message — cleaniq services",
                            ...
                            },
                            {
                              order: 5,
                              subject:
                                "Still here if you need us — cleaniq services",
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={data.fromName}
                      onChange={(e) =>
                        setData((d) => ({ ...d, fromName: e.target.value }))
                      }
                    ...
                    {[
                      {
                        value: "branded",
                        label: "Branded",
                        desc: "Logo, header, reply button & footer",
                        icon: "🎨",
                      },
                      {
                        value: "plain",
                        label: "Plain text",
                        desc: "Just your message — no logo or branding",
                        icon: "✉️",
                      },
                    ].map((opt) => {
                      ...
                      return (
                        <div
                          key={opt.value}
                          onClick={() =>
                            setData((d) => ({ ...d, emailStyle: opt.value }))
                          }
                        ...
                        >
                          <div
                            className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${on ? "bg-emerald-500 border-emerald-500" : "border-white/20"}`}
                          >
                            {on && (
                          ...
                          <div>
                            <p
                              className={`text-sm font-bold ${on ? "text-white" : "text-white/60"}`}
                            >
                              {opt.icon} {opt.label}
                            </p>
                  ...
                  {mailboxes.length === 0 ? (
                    <div className="flex items-center gap-3 bg-amber-500/[0.07] border border-amber-500/20 rounded-xl px-4 py-3.5">
                      ...
                      <p className="text-amber-400/90 text-sm">
                        No Gmail accounts connected. Go to Mailboxes tab first.
                      </p>
                    </div>
                  ) : (
                    mailboxes.map((box) => {
                      ...
                      const pct =
                        box.dailyLimit > 0
                          ? Math.round((box.sentToday / box.dailyLimit) * 100)
                          : 0;
                      return (
                        <div
                          key={box._id}
                        ...
                        >
                          <div
                            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${on ? "bg-emerald-500 border-emerald-500" : "border-white/20"}`}
                          >
                            {on && (
                              <svg
                                width="9"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-[10px] font-black px-1.5 py-0.5 rounded-full uppercase ${STATUS_STYLES[box.status]?.badge || STATUS_STYLES.draft.badge}`}
                              >
                                {box.status}
                              </span>
                            </div>
                            <div className="flex items-center gap-2 mt-1.5">
                              <div className="flex-1 h-1 bg-white/[0.06] rounded-full overflow-hidden max-w-[72px]">
                                <div
                                  className={`h-full rounded-full ${pct >= 100 ? "bg-rose-500" : pct >= 80 ? "bg-amber-400" : "bg-emerald-500"}`}
                                  style={{ width: `${Math.min(100, pct)}%` }}
                                />
                              </div>
                              <p className="text-white/25 text-[11px]">
                                {box.sentToday}/{box.dailyLimit} today
                              </p>
                            </div>
                          </div>
                        </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-white font-bold text-sm">
                      {data.contactIds.length > 0 ? (
                        <>
                          {data.contactIds.length.toLocaleString()}{" "}
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <button
                      onClick={() => setData((p) => ({ ...p, contactIds: [] }))}
                      className="text-[11px] text-white/25 hover:text-white/55 cursor-pointer font-bold transition-all"
                    >
                      Clear
                    </button>
                  </div>
                ...
                <input
                  ...
                  placeholder="Search name, email, company or domain…"
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl max-h-64 overflow-y-auto">
                  {contacts.map((c) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-[18px] h-[18px] rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${sel ? "bg-emerald-500 border-emerald-500" : "border-white/20"}`}
                        >
                          {sel && (
                            <svg
                              width="9"
                        ...
                        <div className="flex flex-col items-end text-right shrink-0">
                          {c.company && (
                            <span className="text-white/20 text-xs truncate max-w-[100px] hidden sm:block">
                              {c.company}
                            </span>
                          )}
                          {c.domain && (
                            <span className="text-emerald-400/60 text-[11px] truncate max-w-[100px] hidden sm:block">
                              {c.domain}
                            </span>
                          )}
                        </div>
                ...
                <div className="flex items-center justify-between text-xs text-white/25">
                  ...
                  <div className="flex gap-1.5">
                    <button
                      ...
                      disabled={cPage === 1}
                      className="px-2.5 py-1 rounded-lg bg-white/[0.04] border border-white/[0.06] disabled:opacity-30 cursor-pointer hover:bg-white/[0.07]"
                    >
                      ← Prev
                    </button>
                    <button
              ...
              <div className="space-y-4">
                <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl overflow-hidden divide-y divide-white/[0.05]">
                  {[
                    { icon: "📋", label: "Campaign name", value: data.name },
                    {
                      icon: "👤",
                      label: "From name",
                      value: data.fromName || "Defaults to mailbox email",
                      dim: !data.fromName,
                    },
                    {
                      icon: "🎨",
                    ...
                    },
                    {
                      icon: "📬",
                      label: "Mailboxes",
                      value: `${data.mailboxIds.length} selected`,
                    },
                    {
                      icon: "✉️",
                      label: "Email sequence",
                      value: `${data.steps.length} email${data.steps.length > 1 ? "s" : ""}`,
                    },
                    {
                      icon: "👥",
                      label: "Recipients",
                      value: `${data.contactIds.length.toLocaleString()} contacts`,
                    },
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-bold ${dim ? "text-white/25" : "text-white"}`}
                      >
                        {value}
                      </span>
                    </div>
                ...
                <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-5">
                  ...
                  <p className="text-white/70 text-sm font-semibold mb-1.5">
                    {data.steps[0]?.subject || "(no subject)"}
                  </p>
                  <p className="text-white/35 text-sm leading-relaxed line-clamp-4 whitespace-pre-wrap">
                    {data.steps[0]?.body || "(empty)"}
                  </p>
                </div>
                ...
                <div className="bg-amber-500/[0.06] border border-amber-500/[0.15] rounded-xl px-4 py-3.5">
                  <p className="text-amber-400 text-xs font-bold flex items-center gap-2 mb-1.5">
                    <AlertCircle size={13} /> Before launching
                  </p>
                  ...
                  <button
                    onClick={() => {
                      if (scheduleMode && !scheduleDate) {
                        setError(
                          "Please pick a date and time to schedule the campaign.",
                        );
                        return;
                      }
                      save(true);
                  ...
                  >
                    {saving ? (
                      <>
                        ...
                        {scheduleMode ? "Scheduling…" : "Launching…"}
                      </>
                    ) : scheduleMode ? (
                      <>
                        <svg
      ...
      {previewIdx !== null && (
        <PreviewModal
          emailStep={data.steps[previewIdx]}
          emailStyle={data.emailStyle}
...
function CampaignCard({ campaign, onOpen, onRefresh, onEdit }) {
  ...
  async function action(verb, e) {
    e.stopPropagation();
    setBusy(verb);
    await afetch(`${API}/cold-email/campaigns/${campaign._id}/${verb}`, {
      method: "POST",
    });
    onRefresh();
    setBusy("");
  ...
  async function del(e) {
    ...
    setBusy("delete");
    await afetch(`${API}/cold-email/campaigns/${campaign._id}`, {
      method: "DELETE",
    });
    onRefresh();
  }
  ...
  const sentPct =
    total > 0 ? Math.min(100, Math.round((s.sent / total) * 100)) : 0;
  const st = STATUS_STYLES[campaign.status] || STATUS_STYLES.draft;
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5 flex-wrap mb-1.5">
            ...
            <span
              className={`inline-flex items-center gap-1 text-[10px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider ${st.badge}`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${st.dot}`} />
              {campaign.status}
            </span>
          ...
          {snippet && (
            <p className="text-white/25 text-xs leading-relaxed line-clamp-1">
              {snippet}
            </p>
          )}
        </div>
        ...
        >
          ...
          {(campaign.status === "active" || campaign.status === "paused") && (
            <button
              onClick={(e) =>
                action(campaign.status === "active" ? "pause" : "resume", e)
              }
              disabled={!!busy}
      ...
      <div className="flex items-end justify-between gap-4 mt-4">
        <div className="flex items-center gap-5">
          <div>
            <p className="text-white/80 font-black text-2xl leading-none tabular-nums">
          ...
          {replyRate !== null && (
            <div>
              <p className="text-emerald-400 font-black text-2xl leading-none tabular-nums">
                {replyRate}%
              </p>
          ...
          <div>
            <p className="text-white/40 font-black text-2xl leading-none tabular-nums">
          ...
          {campaign.steps?.length > 1 && (
            <div>
              <p className="text-white/40 font-black text-2xl leading-none">
        ...
        <div className="flex items-center gap-2 text-white/20 text-xs">
          {campaign.status === "scheduled" && campaign.scheduledStartAt ? (
            <span className="text-violet-400/60">
              Sends{" "}
              {new Date(campaign.scheduledStartAt).toLocaleDateString("en-GB", {
                day: "numeric",
                month: "short",
                hour: "2-digit",
                minute: "2-digit",
      ...
      {sentPct > 0 && (
        <div className="mt-4 h-1 bg-white/[0.04] rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full ${sentPct >= 100 ? "bg-blue-500/50" : "bg-emerald-500/45"}`}
            style={{ width: `${sentPct}%` }}
          />
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-white/35 text-sm mt-0.5">
            {campaigns.length} total · {totalSent.toLocaleString()} sent ·{" "}
            {totalReplied.toLocaleString()} replied
          </p>
        </div>
        <button
          onClick={() => {
          ...
          count > 0 || id === "all" ? (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold cursor-pointer transition-all ${
                filter === id
                  ? "bg-emerald-500 text-white"
                  : "bg-white/[0.04] text-white/35 border border-white/[0.06] hover:text-white/70"
              }`}
            >
              {label}
              <span
                className={`text-[10px] font-black px-1.5 py-0.5 rounded-full ${filter === id ? "bg-white/20 text-white" : "bg-white/[0.06] text-white/35"}`}
              >
                {count}
              </span>
            </button>
