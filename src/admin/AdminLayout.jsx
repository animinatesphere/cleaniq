...
import { 
  ...
  Menu, X, Search, Bell, User, Clock, CheckCircle2, Briefcase, MessageSquare, BookOpen
} from 'lucide-react';
...
const AdminLayout = () => {
  ...
  const menuItems = [
    ...
    { name: 'Chat Support', path: '/admin/chat', icon: <MessageSquare size={20} /> },
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      {isSidebarOpen && <div className="fixed inset-0 bg-primary-dark/50 backdrop-blur-sm z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />}

      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-primary-dark text-white p-6 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:static ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center justify-between mb-12">
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white/50 hover:text-white"><X size={24} /></button>
        </div>

        <nav className="flex-1 space-y-2">
          {menuItems.map((item) => (
            <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={`flex items-center justify-between p-4 rounded-2xl transition-all duration-300 ${location.pathname === item.path ? 'bg-secondary text-primary-dark shadow-lg' : 'hover:bg-white/5 text-slate-400 hover:text-white'}`}>
              <div className="flex items-center gap-3"><span>{item.icon}</span><span className="font-bold text-sm">{item.name}</span></div>
        ...
        <div className="mt-auto border-t border-white/10 pt-6 space-y-4">
          <button onClick={handleLogout} className="flex items-center gap-3 w-full p-4 text-slate-400 hover:text-red-400 transition-colors"><LogOut size={20} /><span className="font-bold text-sm">Logout</span></button>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-slate-200 px-6 lg:px-10 py-4 flex items-center justify-between">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 rounded-xl bg-slate-100 text-slate-600"><Menu size={24} /></button>
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-6 relative" ref={notifRef}>
            ...
            >
              <Bell size={20} />
              {notifications.length > 0 && <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-rose-500 rounded-full border-2 border-white" />}
            </button>
            ...
            {isNotifOpen && (
              <div className="absolute top-full right-0 mt-4 w-[85vw] sm:w-80 bg-white border border-slate-200 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  {notifications.map((n, i) => (
                    ...
                    >
                      <div className="flex gap-3">
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${n.type === 'booking' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}>
                          {n.type === 'booking' ? <CheckCircle2 size={18} /> : <Users size={18} />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-black text-primary-dark truncate">{n.title}</p>
                          <p className="text-[11px] text-slate-500 font-medium line-clamp-2">{n.desc}</p>
                ...
                <button onClick={() => navigate('/admin/bookings')} className="w-full py-4 text-[10px] font-black text-primary uppercase tracking-widest bg-slate-50/50 hover:bg-slate-100 transition-all border-t border-slate-100">
                  View All Activities
                </button>
              </div>
            )}
            
            <div className="flex items-center gap-3 pl-2 group cursor-pointer" onClick={() => navigate('/admin/settings')}>
              <div className="text-right hidden sm:block">
                <p className="text-sm font-black text-primary-dark leading-tight group-hover:text-primary transition-colors">Admin</p>
