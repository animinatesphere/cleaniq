            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map(a => (
                <tr key={a._id} className="group hover:bg-slate-50/50 transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-primary font-black tracking-widest">{a.phone}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-6">
                    <span className={`px-3 py-1.5 rounded-xl text-[10px] font-black border uppercase tracking-tight shadow-sm ${getStatusStyle(a.status || 'Applied')}`}>
                      {a.status || 'Applied'}
                    </span>
                  </td>
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border-4 border-white">
            ...
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-2xl font-black text-primary-dark tracking-tighter">{isEditing ? 'Modify Profile' : 'Talent Profile'}</h3>
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest">Applicant Intelligence</p>
                </div>
              </div>
              ...
              {isEditing ? (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-slate-400 ml-4 uppercase">Full Name</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-slate-400 ml-4 uppercase">Email</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-slate-400 ml-4 uppercase">Phone</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-slate-400 ml-4 uppercase">Status</label>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 ml-4 uppercase">Role / Experience</label>
                ...
                <div className="space-y-10">
                  <div className="flex items-center gap-6">
                    <div className="w-20 h-20 rounded-[32px] bg-primary/10 text-primary flex items-center justify-center text-3xl font-black">
                      {selectedApplicant.fullName?.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-3xl font-black text-primary-dark tracking-tighter">{selectedApplicant.fullName}</h4>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {selectedApplicant.cvPath && (
                      <a href={`https://api.cleaniqservices.com/uploads/${selectedApplicant.cvPath.split('/').pop()}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-3xl bg-primary/5 text-primary font-black uppercase text-[10px] tracking-widest hover:bg-primary hover:text-white transition-all">
                        <FileText size={18} /> View Resume
                      </a>
                    )}
                    {selectedApplicant.idPath && (
                      <a href={`https://api.cleaniqservices.com/uploads/${selectedApplicant.idPath.split('/').pop()}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 rounded-3xl bg-primary/5 text-primary font-black uppercase text-[10px] tracking-widest hover:bg-primary hover:text-white transition-all">
            ...
            <div className="p-8 border-t border-slate-100 bg-slate-50/50 flex gap-4">
              {isEditing ? (
                <>
                  ...
                  <button onClick={() => handleUpdate()} className="flex-1 py-5 rounded-3xl bg-primary text-white text-xs font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-all">
                    <Save size={18} /> Save Profile
                  </button>
                </>
              ) : (
                <>
                  <button onClick={() => { if(window.confirm('Reject this applicant?')) handleUpdate({...selectedApplicant, status: 'Rejected'}); }} className="flex-1 py-5 rounded-3xl bg-rose-50 text-rose-600 border border-rose-100 text-xs font-black uppercase tracking-widest hover:bg-rose-100 transition-all">Reject</button>
                  <button onClick={() => setIsEditing(true)} className="flex-1 py-5 rounded-3xl bg-primary text-white text-xs font-black uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-all">
