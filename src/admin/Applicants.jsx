...
const DocLink = ({ path, label, icon: Icon }) => {
  ...
  if (!url) return (
    <div className="flex items-center gap-2 py-3 px-4 rounded-xl bg-slate-50 border border-slate-100 text-slate-300 text-xs font-bold">
      <AlertTriangle size={14} /> {label} — Not uploaded
    </div>
  );
  return (
    <a href={url} target="_blank" rel="noreferrer"
      className="flex items-center gap-2 py-3 px-4 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-bold hover:bg-emerald-100 transition-all">
  ...
  return (
    <div className="space-y-6 pb-20 relative">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h1 className="text-3xl font-bold text-primary-dark tracking-tighter">Recruitment</h1>
          <p className="text-slate-400 font-bold text-sm mt-1">Review applications & hire workers</p>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={downloadCSV} className="btn-secondary px-6 py-3 rounded-2xl flex items-center gap-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Pending Review', value: applicants.filter(a => ['Applied','Pending','Interviewing','Background Check'].includes(a.status)).length, color: 'text-amber-500', icon: <Clock size={20}/> },
          { label: 'Rejected', value: applicants.filter(a => a.status === 'Rejected').length, color: 'text-rose-500', icon: <XCircle size={20}/> },
        ].map((s, i) => (
      ...
      <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="p-12 text-center text-slate-400 font-bold text-sm">Loading applicants…</div>
        ) : !filtered.length ? (
          <div className="p-12 text-center text-slate-400 font-bold text-sm">No applicants found</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map(a => (
                  <tr key={a._id} className="group hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => { setSelectedApplicant(a); setEditData(a); setIsEditing(false); }}>
                    ...
                    <td className="px-4 py-6">
                      <div className="flex gap-1 flex-wrap">
                        {[
                          ...
                          <span key={d.label} className={`px-2 py-0.5 rounded text-[10px] font-bold border ${d.path ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-slate-50 text-slate-300 border-slate-100'}`}>
                            {d.label}
                          </span>
                        ))}
                      </div>
      ...
      {selectedApplicant && (
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-primary-dark/60 backdrop-blur-md" onClick={() => { setSelectedApplicant(null); setIsEditing(false); }}/>
          <div className="relative w-full max-w-2xl bg-white rounded-2xl shadow-2xl overflow-hidden border-4 border-white">
            ...
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-primary-dark">{isEditing ? 'Edit Application' : 'Application Review'}</h3>
            ...
            <div className="p-6 space-y-6 max-h-[65vh] overflow-y-auto">
              {isEditing ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-slate-400 uppercase">Experience / Notes</label>
                    <textarea value={editData.experience || ''} onChange={e => setEditData({ ...editData, experience: e.target.value })}
                      className="w-full p-3 rounded-xl bg-slate-50 border border-slate-200 font-bold text-sm h-24 resize-none"/>
                  </div>
                </div>
              ) : (
                <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h4 className="text-2xl font-bold text-primary-dark">{selectedApplicant.fullName}</h4>
                      <p className="text-sm text-slate-400">{selectedApplicant.city} · {selectedApplicant.region}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                      ...
                      <p className="text-sm font-bold text-slate-700 break-all">{selectedApplicant.email}</p>
                    </div>
                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                  ...
                  {selectedApplicant.rightToWorkCode && (
                    <div className="p-4 rounded-xl bg-amber-50 border border-amber-100">
                      <p className="text-[10px] font-bold text-amber-600 mb-1">RIGHT TO WORK SHARE CODE</p>
                      <p className="text-base font-bold tracking-widest text-amber-800">{selectedApplicant.rightToWorkCode}</p>
                      <a href={`https://www.gov.uk/prove-right-to-work`} target="_blank" rel="noreferrer"
                        className="text-[10px] text-amber-600 underline mt-1 inline-block">Verify on gov.uk →</a>
                    </div>
                  )}
                  ...
                  {selectedApplicant.experience && (
                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 mb-2">Experience</p>
                  ...
                  {/* Documents */}
                  <div>
                    <p className="text-[10px] font-bold text-slate-400 uppercase mb-3">Uploaded Documents</p>
                  ...
                  {selectedApplicant.additionalNotes && (
                    <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                      <p className="text-[10px] font-bold text-slate-400 mb-2">Additional Notes</p>
                      <p className="text-sm text-slate-600">{selectedApplicant.additionalNotes}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex gap-3 flex-wrap">
              {isEditing ? (
                <>
                  <button onClick={() => setIsEditing(false)} className="flex-1 py-4 rounded-2xl bg-white border border-slate-200 text-xs font-bold text-slate-500">
                    Discard
                  </button>
                  <button onClick={() => handleUpdate()} className="flex-1 py-4 rounded-2xl bg-primary text-white text-xs font-bold flex items-center justify-center gap-2">
                ...
                <>
                  <button onClick={() => { if (window.confirm('Reject this applicant?')) handleUpdate({ ...selectedApplicant, status: 'Rejected' }); }}
                    className="px-5 py-4 rounded-2xl bg-rose-50 text-rose-600 border border-rose-100 text-xs font-bold hover:bg-rose-100 transition-all">
                    Reject
                  </button>
                  <button onClick={() => setIsEditing(true)}
                    className="px-5 py-4 rounded-2xl bg-white border border-slate-200 text-xs font-bold flex items-center gap-2">
                    <Edit3 size={14}/> Edit
                  </button>
                  {selectedApplicant.status !== 'Hired' && (
                    <button onClick={handleHire} disabled={hiring}
                      className="flex-1 py-4 rounded-2xl bg-emerald-600 text-white text-xs font-bold flex items-center justify-center gap-2 hover:bg-emerald-700 transition-all disabled:opacity-60">
                      {hiring ? 'Creating account…' : <><UserPlus size={16}/> Hire & Create Worker Account</>}
                    </button>
                  )}
                  {selectedApplicant.status === 'Hired' && (
                    <div className="flex-1 py-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold flex items-center justify-center gap-2">
