...
const verifyOwnership = async (bookingId, customerEmail) => {
  ...
  if ((booking.customer.email || '').toLowerCase() !== customerEmail.toLowerCase()) return null;
  return booking;
};
...
router.get('/:bookingId', verifyCustomer, async (req, res) => {
  try {
    const booking = await verifyOwnership(req.params.bookingId, req.customer.email);
    if (!booking) return res.status(403).json({ message: 'Access denied.' });

    const messages = await CustomerMessage.find({ bookingId: req.params.bookingId })
      .sort({ createdAt: 1 })
      .limit(200);
    res.json(messages);
  } catch (err) {
...
router.post('/:bookingId/admin-reply', async (req, res) => {
  try {
    ...
    const existingMsg = await CustomerMessage.findOne({ bookingId: req.params.bookingId });
...
router.get('/admin/threads', async (req, res) => {
  try {
    ...
    const enriched = await Promise.all(threads.map(async (t) => {
      const booking = await Booking.findOne({ bookingId: t._id }).select('service schedule.date status customer assignedWorkerName').lean();
      return { ...t, booking };
    }));
