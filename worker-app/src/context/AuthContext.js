...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (email, password) => {
    try {
      const response = await axios.post(`${API_URL}/workers/login`, { email, password });
      ...
      return { 
        success: false, 
        message: error.response?.data?.error || 'Unable to connect to the server. Check your internet.' 
      };
    }
  };
  ...
  return (
    <AuthContext.Provider value={{ login, logout, isLoading, userToken, workerInfo }}>
      {children}
